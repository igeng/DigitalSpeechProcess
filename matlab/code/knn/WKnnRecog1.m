...
for g=1:40 % n_1 各个类别样本数
    ...
    for j=1:140 % n_1_j 样本的第j维度
        for h=1:40 %该类别的其他所有样本点
            %找出在第j维特征上与样本n_1的第j维特征差值最小的样本n_g_j
...
%  for i=1:k
%      if(label(i)==1)
%          sumlabel(1)=sumlabel(1)+1;
%      elseif(label(i)==2)
%          sumlabel(2)=sumlabel(2)+1;
%      elseif(label(i)==3)
%          sumlabel(3)=sumlabel(3)+1;
%      elseif(label(i)==4)
%          sumlabel(4)=sumlabel(4)+1;
%      else
%          sumlabel(5)=sumlabel(5)+1;
%      end
%  end
%  
%  weight=weight.*sumlabel;
