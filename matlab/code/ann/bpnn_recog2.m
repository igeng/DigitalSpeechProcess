...
%5折验证
angerVec(:,[1:10 41:50]) = angerVec(:,[41:50 1:10]);
hapVec(:,[1:10 41:50]) = hapVec(:,[41:50 1:10]);
neutralVec(:,[1:10 41:50]) = neutralVec(:,[41:50 1:10]);
sadnessVec(:,[1:10 41:50]) = sadnessVec(:,[41:50 1:10]);
fearVec(:,[1:10 41:50]) = fearVec(:,[41:50 1:10]);
...
[sum2,bpnnvote2,Y2]=bpnn(trainsample,testsample,class);

figure(1)
x=1:1:5;
plot(x,sum2,'-*r'); %线性，颜色，标记
ylim([0,1])
...
%给图上加说明文字
for i = 1:length(sum2)
    text(n(i),sum2(i)-0.03,num2str(sum2(i)));
end
title('BPNN 算法识别结果 ');
xlabel('情感类别')
ylabel('识别率')
save('sum2.mat','sum2');
save('bpnnvote2.mat','bpnnvote2');