...
def extract_gfcc(in_path, file):
    ...
    # S = gtgram(y, sr, 0.128, 0.032, 256, 100)
    S = gtgram(y, sr, 0.025, 0.010, 256, 100)
    ...
    plt.yticks([])
    plt.tight_layout()
    plt.savefig('./myRecording/test/' + number + '/' + file[:-3] + 'png', bbox_inches='tight',
                pad_inches=-0.1)
    plt.close()
    return
...
if __name__ == '__main__':
    number = '0'
    in_path = './myRecording/audio_tr_10db/' + number + '/'
    file = '0_Agnes_40.wav'
    extract_gfcc(in_path=in_path, file=file)
    ...
    #     S = extract_gfcc(in_path, wavfile, 256)
    #
    ...
    #gammatone frequency figure 1
    # data, fs = sf.read('/home/igeng/PycharmProjects/Spoken_Number_Recognition_gfcc/codes/myRecording/audio_tr/2/2_Agnes_40.wav')
    # y = gtgram(data, fs, 0.025, 0.010, 64, 50)
    # print(y)
    # print(y.shape)
    # plt.figure()
    # plt.imshow(y)
